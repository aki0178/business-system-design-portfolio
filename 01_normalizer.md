...
> 「フォーマットがバラバラな契約書類」から、機種ごとの調達数・契約数を抽出・集計し、社内標準の「治具一覧表（仮称）」へ自動転記する業務ツール。
>
...
書式が毎回異なる契約書類を、列位置検出・ヘッダ展開・機種名正規化の3段で吸収し、社内標準の一覧表へ自動転記する。手作業では1件あたり数十分かかる照合・集計を、検証ステップ付きで自動化した。
...
- 同じ「調達数」「契約数」でも、**列位置・ヘッダ階層が書類ごとに違う**（マージ・段組み・別名表記）
- 機種名（部品名）の表記揺れが激しい（`A100 XX R` / `A100-XX-R(US)` / `A100 XX L (対応01)` などがすべて同一機種。※実際の型番は仮称化）
- 「機種名列」が常にC列にあるとは限らず、**データ開始行も書類ごとに違う**
- 転記先の一覧表側も、ヘッダ名が完全一致するとは限らない（`調達数` → `調達数量`、`契約数` → `契約数量` など）
- 企画台数の元値が別ファイル（生産見積表（仮称））に分散しており、**ファイル名から該当ファイルを特定する必要がある**
...
4. 展開後のヘッダから、ユーザー指定の `調達数/契約数` を含む列を検索
...
※本データの帳票名・機種名は、説明のため一部を仮称・抽象化しています。
